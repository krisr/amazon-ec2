...
This 'amazon-ec2' Ruby Gem is an interface library that can be used to interact with the Amazon EC2 and ELB systems and control server resources on demand from your Ruby scripts, or from applications written in your Ruby framework of choice (Ruby on Rails, Merb, etc.).
...
The library exposes two main interface classes:

* AWS::EC2::Base
* AWS::ELB::Base

It is through an instance of these classes that you will perform all the operations for using the EC2 and ELB services including query string header signing.

The public methods on these classes closely mirror their respective Amazon Query APIs, and as such the Query API Reference in the EC2 Developer Guide ( http://developer.amazonwebservices.com/connect/kbcategory.jspa?categoryID=84 ) will prove helpful.
...
The library exposes two main interface modules

  AWS::EC2::Base
  AWS::ELB::Base

These objects requires arguments which include your AWS credentials and will return an object that you can use to make method calls directly against EC2 and ELB respectively. All the operations for using these Amazon services, including query string header signing, are handled automatically for you. The connection string will look something like this:
...
=== Ruby script usage example for ec2:
...
=== Ruby script usage example for elb:

This code should create a new load balancer in the east-1a availability zone that will forward HTTP traffic on port 80 to port 8080 on any instances that are later registered with it.
  ...
  resp = elb.create_load_balancer({
    ...
    :listeners => [{:protocol => 'HTTP', :load_balancer_port => '80', :instance_port => '8080'}]
  })

  puts "Load balancer created : #{resp.DNSName}"
